...
# Function to predict emotion based on valence and arousal
def predict_emotion(valence, arousal):
    if valence > 0.5 and arousal > 0.5:
        ...
        return "Angry"
    elif 0.3 <= valence <= 0.7 and 0.3 <= arousal <= 0.7:
        return "Neutral"
    else:
        return "Surprised"
...
st.title("Arousal, Valence, and Emotion Prediction from Webcam")
...
while True:
    ...
    if arousal is not None and valence is not None:
        ...
        filename = f"frame_{timestamp}_arousal_{arousal:.2f}_valence_{valence:.2f}_emotion_{emotion}.jpg"
        cv2.imwrite(os.path.join(output_dir, filename), frame)
        
        # Append prediction to the list
        predictions.append({'Timestamp': timestamp[:-3], 'Arousal': arousal, 'Valence': valence, 'Emotion': emotion})
